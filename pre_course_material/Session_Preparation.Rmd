---
title: "Pre-course material - HARMONY - Grant Period 5 - Larissa, Greece Training School 22 - 24 April 2024"
author:
- Eleftherios Meletis, Xanthi Rousou, Matthew Denwood, Polychronis Kostoulas
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
  pdf_document: default
subtitle: Homework and software installation for the training school on Bayesian methods
fontsize: 12pt
---
...
We have created a public repository containing the teaching material for the training workshop. This means that anyone can view the teaching material at any time via the following website: https://github.com/LefMel/harmony_larissa_2024
...
The easiest way to clone the `harmony_larissa_2024` repository on to your computer is via RStudio. Go to the `File` menu and select `New project`. It will open and wizard menu, giving you three alternative ways to create a project: select `Version Control` and then `GIT`. Then type/paste `https://github.com/LefMel/harmony_larissa_2024.git` into the box where it says `Repository URL`. A suggested local path folder will be created automatically at the bottom but feel free to change this. Then click on `Clone` and wait for the files to download.
